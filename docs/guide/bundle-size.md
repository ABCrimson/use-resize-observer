...
One of the primary design goals of `@crimson_dev/use-resize-observer` is an aggressively small bundle. The full main entry — pool, scheduler, and hook — weighs just 1.04 kB min+gzip.

## Size Budget

| Entry Point | Min+Gzip | Description |
|------------|----------|-------------|
| Main (`useResizeObserver`) | **1.04 kB** | Full hook with pool + scheduler |
| Core (`/core`) | **363 B** | EventTarget-based, framework-agnostic |
| Server (`/server`) | **121 B** | SSR/RSC no-op entry |
| Shim (`/shim`) | **531 B** | Legacy browser polyfill |
| Worker (`/worker`) | **1.32 kB** | Off-main-thread via SharedArrayBuffer |
...
```tsx
// Only the main hook is bundled (1.04 kB)
...
|---------|----------|-------------|-----|
| **@crimson_dev/use-resize-observer** | **1.04 kB** | **0** | **Yes** |
...
If you use the [Import Cost](https://marketplace.visualstudio.com/items?itemName=wix.vscode-import-cost) VS Code extension, you will see the size inline next to your import statement. The main hook should display as approximately 1.04 kB gzip.
...
- [Worker Mode](/guide/worker) -- The additional 1.32 kB for off-main-thread support
