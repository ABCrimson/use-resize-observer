...
|------------|----------|-------------|
| Main (`useResizeObserver`) | **1.12 kB** | Full hook with pool + scheduler |
| Core (`/core`) | **350 B** | EventTarget-based, framework-agnostic |
| Server (`/server`) | **114 B** | SSR/RSC no-op entry |
| Shim (`/shim`) | **537 B** | Legacy browser polyfill |
| Worker (`/worker`) | **1.21 kB** | SAB-based measurement sharing |
...
|---------|----------|-------------|-----|
| **@crimson_dev/use-resize-observer** | **1.12 kB** | **0** | **Yes** |
...
- [Worker Mode](/guide/worker) -- The additional 1.21 kB for SAB-based measurement sharing
