...
describe('extractBoxSize', () => {
  ...
  it('should extract content-box size', () => {
    const size = extractBoxSize(entry, 'content-box');
    expect(size?.inlineSize).toBe(320);
    expect(size?.blockSize).toBe(240);
  });

  it('should extract border-box size', () => {
    const size = extractBoxSize(entry, 'border-box');
    expect(size?.inlineSize).toBe(320);
    expect(size?.blockSize).toBe(240);
  });

  it('should extract device-pixel-content-box size', () => {
    const size = extractBoxSize(entry, 'device-pixel-content-box');
    expect(size).toBeDefined();
    // Mock uses width * devicePixelRatio (default 1), so values match content-box
  ...
  it('should fallback to contentBoxSize when devicePixelContentBoxSize is missing', () => {
    const entryNoDevice: ResizeObserverEntry = {
      ...entry,
      devicePixelContentBoxSize: undefined as unknown as ReadonlyArray<ResizeObserverSize>,
    };
    const size = extractBoxSize(entryNoDevice, 'device-pixel-content-box');
    expect(size?.inlineSize).toBe(320);
  });
});
...
describe('extractDimensions', () => {
  ...
  it('should return width and height for content-box', () => {
    ...
    const { width, height } = extractDimensions(entry, 'content-box');
  ...
  it('should return width and height for border-box', () => {
    ...
    const { width, height } = extractDimensions(entry, 'border-box');
    expect(width).toBe(1024);
    expect(height).toBe(768);
  });

  it('should return 0 for missing size entries', () => {
    const emptyEntry: ResizeObserverEntry = {
      target: el,
    ...
    const { width, height } = extractDimensions(emptyEntry, 'content-box');
    expect(width).toBe(0);
