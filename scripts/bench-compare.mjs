...
const pr = existsSync(PR_PATH)
  ? JSON.parse(readFileSync(PR_PATH, 'utf8'))
  : { benchmarks: {} };
...
for (const [name, prResult] of Object.entries(pr.benchmarks ?? {})) {
  ...
  if (change < -10) {
    hasRegression = true;
    comparisons.push({ name, status: 'regression', change: change.toFixed(1), baseResult, prResult });
  } else if (change > 10) {
    comparisons.push({ name, status: 'improvement', change: change.toFixed(1), baseResult, prResult });
  } else {
...
const markdown = [
  ...
  '|-----------|--------|--------|',
  ...comparisons.map(
    (c) => `| ${c.name} | ${c.status === 'regression' ? '⚠️' : c.status === 'improvement' ? '✅' : '➖'} ${c.status} | ${c.change ?? 'N/A'}% |`,
  ),
  '',
  hasRegression ? '> ⚠️ **Performance regressions detected.** Please investigate.' : '> ✅ No regressions detected.',
].join('\n');
